...
@section('content')
    ...
    <a href="{{ route('gallery.create') }}" class="btn btn-sm mb-4 my-4"><i class='bx bx-plus'></i> Tambah</a>
                    ...
                    <tr class="bg-white border-b ">
                        <td class="td">
                            ...
                            <img class="object-cover h-auto max-w-[100px] aspect-[1/1]"
                                src="{{ \Storage::url($gallery->path_image) }}" alt="">
                            {{-- </div> --}}
                        </td>
                        <td class="td font-medium text-gray-900">{{ $gallery->title }}</td>
                        <td class="td flex space-x-2 justify-end items-center h-full">
                            <a class="btn btn-sm" href="{{ route('gallery.edit', $gallery->id) }}">Edit</a>
                            <form action="{{ route('gallery.destroy', $gallery->id) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-sm btn-outline" type="submit"
                                    onClick="javascript: return confirm('Foto akan dihapus ?');">Delete</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
