    ...
    <form action="{{ route('post.update', $post->id) }}" method="post" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 mt-4 place-items-start gap-4">
            ...
            <div class="aspect-[8/6] rounded-xl overflow-hidden w-full bg-gray-100">
                <img class="w-full h-full object-cover" src="{{ \Storage::url($post->thumbnail) }}"
        ...
        <div class="flex flex-wrap gap-4 mt-4">
            @foreach ($tags as $tag)
                @php
                    $checked = in_array($tag->id, $post->tags->pluck('tag_id')->toArray()) ? 'checked' : '';
                @endphp
                <label for="{{ $tag->id }}"
                    ...
                    <input type="checkbox" name="tags[]" id="{{ $tag->id }}" value="{{ $tag->id }}"
                        class="checkbox cursor-pointer" {{ $checked }}>
