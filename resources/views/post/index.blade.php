...
@section('content')
    ...
    <a href="{{ route('post.create') }}" class="btn btn-sm mb-4 my-4"><i class='bx bx-plus'></i> Tambah</a>
            ...
            <tbody class="divide-y-2">
                @foreach ($posts as $post)
                    <tr class="bg-white border-b ">
                        ...
                        <td class="td font-medium text-gray-900">{{ $post->user->name }}
                        </td>
                        <td class="td font-medium text-gray-900">{{ $post->status }}</td>
                        <td class="td flex space-x-2 justify-end">
                            <a href="{{ route('post.show', $post->slug) }}" class="btn btn-sm btn-blue">View</a>
                            <a class="btn btn-sm" href="{{ route('post.edit', $post->id) }}">Edit</a>
                            <form action="{{ route('post.destroy', $post->id) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-sm btn-outline" type="submit"
                                    onClick="javascript: return confirm('Postingan akan dihapus ?');">Delete</button>
                            </form>
                            <form method="post" action="{{ route('post.status', $post->id) }}">
                                @csrf
                                @method('PUT')
                                <button class="btn btn-sm btn-blue-outline" type="submit"
                                    {{ $post->status == 'PUBLISHED' ? 'disabled' : '' }}
                                    onClick="javascript: return confirm('Postingan akan di publish ?');">{{ $post->status == 'PUBLISHED' ? 'Published' : 'Publish' }}</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
