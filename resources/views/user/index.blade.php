...
@section('content')
    ...
    <a href="{{ route('user.create') }}" class="btn btn-sm mb-4 my-4"><i class='bx bx-plus'></i> Tambah</a>
            ...
            <tbody class="divide-y-2">
                @foreach ($users as $user)
                    <tr class="bg-white border-b ">
                        ...
                        <td class="td font-medium text-gray-900">{{ $user->email }}</td>
                        <td class="td flex space-x-2 justify-end">
                            <a class="btn btn-sm" href="{{ route('user.edit', $user->id) }}">Edit</a>
                            <form action="{{ route('user.destroy', $user->id) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-sm btn-outline" type="submit"
                                    onClick="javascript: return confirm('User akan dihapus ?');">Delete</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
