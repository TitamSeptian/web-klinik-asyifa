...
@section('content')
    ...
    <form action="{{ route('service.update', $service->id) }}" method="post">
        ...
        <div class="form-control">
            ...
            <input class="input" type="text" name="title" id="title" value="{{ old('title') ?? $service->title }}"
                required>
            @error('title')
        ...
        <div class="form-control">
            ...
            <span class="text-sm italic">Cari icon di <a href="https://boxicons.com/" class="underline text-blue-500"
                    target="_blank">https://boxicons.com/</a></span>
            <input class="input" type="text" name="icon" id="icon" value="{{ old('icon') ?? $service->icon }}"
                required>
            @error('icon')
                <span class="invalid">{{ $message }}</span>
            @enderror
        ...
        <div class="form-control">
            ...
            <select name="type" id="type" class="input">
                ...
                <option value="lab" {{ old('type') ?? $service->type == 'lab' ? 'selected' : '' }}>Service Labolatorium
                </option>
                <option value="service" {{ old('type') ?? $service->type == 'service' ? 'selected' : '' }}>Service Klinik
                </option>
            </select>
            @error('type')
                <span class="invalid">{{ $message }}</span>
